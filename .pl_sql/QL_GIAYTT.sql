--------------------------------------------------------------------------------
-- PHẦN 1: TẠO BẢNG VÀ DỮ LIỆU CƠ SỞ
--------------------------------------------------------------------------------
drop table DANGNHAP;
drop table CTHD;
DROP TABLE HOADON;
...
--- BẢNG KHÁCH HÀNG, SEQUENCE VÀ TRIGGER
...
BEGIN
    ...
    -- Sửa lỗi logic: '??t c?c' thành 'Đặt cọc'
    IF v_TT NOT IN ('Chưa thanh toán', 'Đặt cọc') THEN
...
-- Sửa tên: N'L  Huy' thành N'Lê Huy'
INSERT INTO NHANVIEN (MaNV, TenNV, SDTNV, NgaySinhNV, DiaChiNV, GioiTinhNV, CCCD, NVL, Luong, MATK)
VALUES ('NV002', N'Lê Huy', '0976543210', TO_DATE('20-05-2000', 'DD-MM-YYYY'), N'Hải Phòng', 'Nam', '0234567890', 2021, 7500000, 'TK002');
...
-- Sửa: 'c?a' thành 'của'
INSERT INTO SANPHAM (MaSP, TenSP, GiaBan, GioiTinh, ThongTinSP, ChatLieu, KichThuoc, SoLuongTon, TinhTrang, MaL)
VALUES ('SP009', N'Anta KT7', 2800000, N'Nam', N'Giày bóng rổ Anta KT7 của Klay Thompson', N'Vải tổng hợp + EVA', N'44', 17, N'Còn hàng', 'L002');
...
INSERT INTO KHACHHANG (TenKH, SDTKH, DiaChiKH, GioiTinhKH, NamSinhKH)
VALUES (N'Lê Thị', '0911223345', N'TP.HCM', N'Nữ', 1999);
...
--------------------------------------------------------------------------------
-- PHẦN 3: BẢO MẬT VÀ MÃ HÓA (ENCRYPTION)
--------------------------------------------------------------------------------


---- Giải Mã AES ----
-- Khai báo package giải mã
...
--------------------------------------------------------------------------------
-- PHẦN 4: HỆ THỐNG GHI NHẬT KÝ VÀ GIẢI TRÌNH (AUDITING)
--------------------------------------------------------------------------------

-- 1. TẠO BẢNG AUDIT_LOG ĐỂ LƯU TRỮ NHẬT KÝ
...
-- 2. TẠO FUNCTION/PROCEDURE HỖ TRỢ GHI NHẬT KÝ
...
CREATE OR REPLACE PACKAGE BODY PKG_AUDIT IS
    ...
    EXCEPTION
        WHEN OTHERS THEN
            -- Không throw exception để không ảnh hưởng đến transaction chính
            NULL;
    END LogAudit;
    ...
    FUNCTION GetOldValues(p_TableName VARCHAR2, p_PrimaryKey VARCHAR2, p_PrimaryKeyValue VARCHAR2) RETURN CLOB IS
        v_Result CLOB;
    BEGIN
        -- Function này có thể được mở rộng để lấy giá trị cũ từ bảng
        RETURN NULL;
    END GetOldValues;
    ...
    FUNCTION GetNewValues(p_TableName VARCHAR2, p_PrimaryKey VARCHAR2, p_PrimaryKeyValue VARCHAR2) RETURN CLOB IS
        v_Result CLOB;
    BEGIN
        -- Function này có thể được mở rộng để lấy giá trị mới từ bảng
        RETURN NULL;
    END GetNewValues;
...
-- Chạy với quyền DBA (SYS hoặc SYSTEM)
...
-- Audit cho bảng LOAISP
AUDIT INSERT, UPDATE, DELETE ON LOAISP BY ACCESS;

-- 12. TẠO PROCEDURE XEM NHẬT KÝ
CREATE OR REPLACE PROCEDURE SP_XEM_NHAT_KY_THEO_BANG(
    p_TableName VARCHAR2,
...
BEGIN
    FOR rec IN (
        ...
        DBMS_OUTPUT.PUT_LINE('ID: ' || rec.AUDIT_ID || 
                            ...
                            ' | Người dùng: ' || rec.USER_NAME || 
                            ' | Thời gian: ' || rec.THOI_GIAN);
    END LOOP;
END;
/

-- 13. TẠO HÀM ĐẾM SỐ LƯỢNG THAO TÁC
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Đã xóa ' || v_DeletedCount || ' bản ghi nhật ký cũ hơn ' || p_DaysOld || ' ngày.');
END;
/
...
--
-- 4. Tạo USER và gán các thành phần trên
...
--
-- 3.3. Thiết lập công cụ FGA, cài đặt VPD, cài đặt OLS
-- Thiết lập FGA
--
-- BEGIN
--  DBMS_FGA.ADD_POLICY(
...
-- END;
--
-- Câu lệnh xem nhật ký FGA:
-- SELECT * FROM DBA_FGA_AUDIT_TRAIL;
--
-- Kiểm tra xem chính sách đã đúng chưa 
-- SELECT POLICY_NAME, OBJECT_NAME, POLICY_COLUMN 
-- FROM DBA_AUDIT_POLICIES 
-- WHERE POLICY_NAME = 'AUDIT_XEM_LUONG';
--
--
-- Cài đặt VPD (Virtual Private Database)
-- 1. Tạo hàm chính sách (Policy Function)
...
-- END;
--
-- 2. Gán chính sách vào bảng NHANVIEN
-- BEGIN
--  DBMS_RLS.ADD_POLICY(
--    object_schema   => 'NgNam',
--    object_name     => 'NHANVIEN',
...
--
--
-- Cài đặt OLS (Oracle Label Security)
-- Kiểm tra trạng thái
-- SELECT VALUE FROM v$option WHERE PARAMETER = 'Oracle Label Security';
--
-- 1. Đăng ký OLS với Database
